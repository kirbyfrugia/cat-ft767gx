...
class RigUtils:
  ...
  def start_cat(self, status_86byte_parser):
    """Enable CAT mode. The provided parser callback will be called with the
    86-byte status response so the caller can update their own state object."""
    if not self.serial_port or not self.serial_port.is_open:
      raise Exception("Serial port not open.")

    print("Enabling CAT mode...")
    command = YaesuCommand("cat enable", YaesuInstruction.CAT_SW, 86, status_86byte_parser, data1=0)
  ...
  def cat_command(self, yaesu_command, expect_status_update=True):
    ...
    if self.serial_port.in_waiting > 0:
      garbage = self.serial_port.read(self.serial_port.in_waiting)
      print(f"Warning: Discarded {len(garbage)} unexpected bytes from input buffer: {garbage}")